...
class OpenGexExporter(bpy.types.Operator, ExportHelper):
	...
	option_float_as_hex : bpy.props.BoolProperty(name = "Use Hexadecimals", description = "Decimal numbers will be exported as hexadecimal numbers", default = False)
	...
	def DeindexMesh(mesh, materialTable, shouldExportVertexColor = True):
		...
		for face in mesh.loop_triangles:
			for i, k in enumerate(face.vertices):
				exportVertex = ExportVertex()
				exportVertex.vertexIndex = k
				exportVertex.faceIndex = faceIndex
				exportVertex.position = vertexArray[k].co
				loop_index = face.loops[i]
				if face.use_smooth:
					n = corner_normals[loop_index]
					exportVertex.normal[0] = n[0]
					exportVertex.normal[1] = n[1]
					exportVertex.normal[2] = n[2]
				else:
		...
		# Vertex colors (Blender 4.4+)
		colorCount = 0
		colorFace = None
		if hasattr(mesh, 'color_attributes') and len(mesh.color_attributes) > 0:
			colorCount = len(mesh.color_attributes)
			if (colorCount > 0 and shouldExportVertexColor):
				color_attr = mesh.color_attributes.active_color if hasattr(mesh.color_attributes, 'active_color') and mesh.color_attributes.active_color else mesh.color_attributes[0]
				if color_attr:
					colorFace = color_attr.data
		if colorFace:
			vertexIndex = 0
			faceIndex = 0
			for face in mesh.loop_triangles:
				cf = colorFace[faceIndex]
				color = getattr(cf, 'color', cf[:])
				exportVertexArray[vertexIndex].color[0] = color[0]
				vertexIndex += 1
				exportVertexArray[vertexIndex].color[1] = color[1]
				vertexIndex += 1
				exportVertexArray[vertexIndex].color[2] = color[2]
				vertexIndex += 1
				if (len(face.vertices) == 4):
					exportVertexArray[vertexIndex].color[0] = color[1]
					vertexIndex += 1
					exportVertexArray[vertexIndex].color[1] = color[2]
					vertexIndex += 1
					exportVertexArray[vertexIndex].color[2] = color[3] if len(color) > 3 else color[2]
					vertexIndex += 1
				faceIndex += 1
