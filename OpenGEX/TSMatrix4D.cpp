...
Matrix4D::Matrix4D(
    float n00, float n01, float n02, float n03, float n10, float n11, float n12, float n13, float n20, float n21, float n22, float n23, float n30, float n31, float n32, float n33)
    : Mat4D<TypeMatrix4D>(n00, n01, n02, n03, n10, n11, n12, n13, n20, n21, n22, n23, n30, n31, n32, n33)
{
}
...
{
    return (Matrix4D(m1(0, 0) * m2(0, 0) + m1(0, 1) * m2(1, 0) + m1(0, 2) * m2(2, 0), m1(0, 0) * m2(0, 1) + m1(0, 1) * m2(1, 1) + m1(0, 2) * m2(2, 1),
                     m1(0, 0) * m2(0, 2) + m1(0, 1) * m2(1, 2) + m1(0, 2) * m2(2, 2), m1(0, 3), m1(1, 0) * m2(0, 0) + m1(1, 1) * m2(1, 0) + m1(1, 2) * m2(2, 0),
                     m1(1, 0) * m2(0, 1) + m1(1, 1) * m2(1, 1) + m1(1, 2) * m2(2, 1), m1(1, 0) * m2(0, 2) + m1(1, 1) * m2(1, 2) + m1(1, 2) * m2(2, 2), m1(1, 3),
                     m1(2, 0) * m2(0, 0) + m1(2, 1) * m2(1, 0) + m1(2, 2) * m2(2, 0), m1(2, 0) * m2(0, 1) + m1(2, 1) * m2(1, 1) + m1(2, 2) * m2(2, 1),
                     m1(2, 0) * m2(0, 2) + m1(2, 1) * m2(1, 2) + m1(2, 2) * m2(2, 2), m1(2, 3), m1(3, 0) * m2(0, 0) + m1(3, 1) * m2(1, 0) + m1(3, 2) * m2(2, 0),
                     m1(3, 0) * m2(0, 1) + m1(3, 1) * m2(1, 1) + m1(3, 2) * m2(2, 1), m1(3, 0) * m2(0, 2) + m1(3, 1) * m2(1, 2) + m1(3, 2) * m2(2, 2), m1(3, 3)));
}

Vector4D Terathon::operator*(const Matrix4D& m, const Vector3D& v)
{
    return (Vector4D(m(0, 0) * v.x + m(0, 1) * v.y + m(0, 2) * v.z, m(1, 0) * v.x + m(1, 1) * v.y + m(1, 2) * v.z, m(2, 0) * v.x + m(2, 1) * v.y + m(2, 2) * v.z,
                     m(3, 0) * v.x + m(3, 1) * v.y + m(3, 2) * v.z));
}

Vector4D Terathon::operator*(const Vector3D& v, const Matrix4D& m)
{
    return (Vector4D(m(0, 0) * v.x + m(1, 0) * v.y + m(2, 0) * v.z, m(0, 1) * v.x + m(1, 1) * v.y + m(2, 1) * v.z, m(0, 2) * v.x + m(1, 2) * v.y + m(2, 2) * v.z,
                     m(0, 3) * v.x + m(1, 3) * v.y + m(2, 3) * v.z));
}

Vector4D Terathon::operator*(const Matrix4D& m, const Point3D& p)
{
    return (Vector4D(m(0, 0) * p.x + m(0, 1) * p.y + m(0, 2) * p.z + m(0, 3), m(1, 0) * p.x + m(1, 1) * p.y + m(1, 2) * p.z + m(1, 3),
                     m(2, 0) * p.x + m(2, 1) * p.y + m(2, 2) * p.z + m(2, 3), m(3, 0) * p.x + m(3, 1) * p.y + m(3, 2) * p.z + m(3, 3)));
}

Vector4D Terathon::operator*(const Point3D& p, const Matrix4D& m)
{
    return (Vector4D(m(0, 0) * p.x + m(1, 0) * p.y + m(2, 0) * p.z + m(3, 0), m(0, 1) * p.x + m(1, 1) * p.y + m(2, 1) * p.z + m(3, 1),
...
{
    return (Vector4D(m(0, 0) * v.x + m(0, 1) * v.y, m(1, 0) * v.x + m(1, 1) * v.y, m(2, 0) * v.x + m(2, 1) * v.y, m(3, 0) * v.x + m(3, 1) * v.y));
}

Vector4D Terathon::operator*(const Vector2D& v, const Matrix4D& m)
{
    return (Vector4D(m(0, 0) * v.x + m(1, 0) * v.y, m(0, 1) * v.x + m(1, 1) * v.y, m(0, 2) * v.x + m(1, 2) * v.y, m(0, 3) * v.x + m(1, 3) * v.y));
}

Vector4D Terathon::operator*(const Matrix4D& m, const Point2D& p)
{
    return (Vector4D(m(0, 0) * p.x + m(0, 1) * p.y + m(0, 3), m(1, 0) * p.x + m(1, 1) * p.y + m(1, 3), m(2, 0) * p.x + m(2, 1) * p.y + m(2, 3),
                     m(3, 0) * p.x + m(3, 1) * p.y + m(3, 3)));
}

Vector4D Terathon::operator*(const Point2D& p, const Matrix4D& m)
{
    return (Vector4D(m(0, 0) * p.x + m(1, 0) * p.y + m(3, 0), m(0, 1) * p.x + m(1, 1) * p.y + m(3, 1), m(0, 2) * p.x + m(1, 2) * p.y + m(3, 2),
...
Matrix4D Terathon::Inverse(const Matrix4D& m)
{
    ...
    return (Matrix4D((b ^ v) + t * y, -(b ^ t), (v ^ a) - t * x, (a ^ t), (d ^ u) + s * w, -(d ^ s), (u ^ c) - s * z, (c ^ s)));
...
Transform3D Transform3D::MakeRotationX(float angle)
{
    ...
    return (Transform3D(1.0F, 0.0F, 0.0F, 0.0F, 0.0F, v.x, -v.y, 0.0F, 0.0F, v.y, v.x, 0.0F));
}
...
Transform3D Transform3D::MakeRotationY(float angle)
{
    ...
    return (Transform3D(v.x, 0.0F, v.y, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, -v.y, 0.0F, v.x, 0.0F));
}
...
Transform3D Transform3D::MakeRotationZ(float angle)
{
    ...
    return (Transform3D(v.x, -v.y, 0.0F, 0.0F, v.y, v.x, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F));
}
...
Transform3D Transform3D::MakeRotation(float angle, const Bivector3D& axis)
{
    ...
    return (Transform3D(c + x * axis.x, axay - s * axis.z, axaz + s * axis.y, 0.0F, axay + s * axis.z, c + y * axis.y, ayaz - s * axis.x, 0.0F, axaz - s * axis.y,
                        ayaz + s * axis.x, c + z * axis.z, 0.0F));
}
...
Transform3D Transform3D::MakeReflection(const Vector3D& a)
{
    ...
    return (Transform3D(x * a.x + 1.0F, axay, axaz, 0.0F, axay, y * a.y + 1.0F, ayaz, 0.0F, axaz, ayaz, z * a.z + 1.0F, 0.0F));
}
...
Transform3D Transform3D::MakeReflection(const Plane3D& plane)
{
    ...
    return (Transform3D(x * plane.x + 1.0F, nxny, nxnz, x * plane.w, nxny, y * plane.y + 1.0F, nynz, y * plane.w, nxnz, nynz, z * plane.z + 1.0F, z * plane.w));
}
...
Transform3D Transform3D::MakeSkew(float angle, const Vector3D& a, const Vector3D& b)
{
    ...
    return (Transform3D(x * b.x + 1.0F, x * b.y, x * b.z, 0.0F, y * b.x, y * b.y + 1.0F, y * b.z, 0.0F, z * b.x, z * b.y, z * b.z + 1.0F, 0.0F));
}
...
Transform3D Terathon::operator*(const Transform3D& m1, const Transform3D& m2)
{
    ...
    return (Transform3D(m1(0, 0) * m2(0, 0) + m1(0, 1) * m2(1, 0) + m1(0, 2) * m2(2, 0), m1(0, 0) * m2(0, 1) + m1(0, 1) * m2(1, 1) + m1(0, 2) * m2(2, 1),
                        m1(0, 0) * m2(0, 2) + m1(0, 1) * m2(1, 2) + m1(0, 2) * m2(2, 2), m1(0, 0) * m2(0, 3) + m1(0, 1) * m2(1, 3) + m1(0, 2) * m2(2, 3) + m1(0, 3),
                        m1(1, 0) * m2(0, 0) + m1(1, 1) * m2(1, 0) + m1(1, 2) * m2(2, 0), m1(1, 0) * m2(0, 1) + m1(1, 1) * m2(1, 1) + m1(1, 2) * m2(2, 1),
                        m1(1, 0) * m2(0, 2) + m1(1, 1) * m2(1, 2) + m1(1, 2) * m2(2, 2), m1(1, 0) * m2(0, 3) + m1(1, 1) * m2(1, 3) + m1(1, 2) * m2(2, 3) + m1(1, 3),
                        m1(2, 0) * m2(0, 0) + m1(2, 1) * m2(1, 0) + m1(2, 2) * m2(2, 0), m1(2, 0) * m2(0, 1) + m1(2, 1) * m2(1, 1) + m1(2, 2) * m2(2, 1),
                        m1(2, 0) * m2(0, 2) + m1(2, 1) * m2(1, 2) + m1(2, 2) * m2(2, 2), m1(2, 0) * m2(0, 3) + m1(2, 1) * m2(1, 3) + m1(2, 2) * m2(2, 3) + m1(2, 3)));
...
Matrix4D Terathon::operator*(const Matrix4D& m1, const Transform3D& m2)
{
    ...
    return (Matrix4D(m1(0, 0) * m2(0, 0) + m1(0, 1) * m2(1, 0) + m1(0, 2) * m2(2, 0), m1(0, 0) * m2(0, 1) + m1(0, 1) * m2(1, 1) + m1(0, 2) * m2(2, 1),
                     ...
                     m1(2, 0) * m2(0, 0) + m1(2, 1) * m2(1, 0) + m1(2, 2) * m2(2, 0), m1(2, 0) * m2(0, 1) + m1(2, 1) * m2(1, 1) + m1(2, 2) * m2(2, 1),
                     m1(2, 0) * m2(0, 2) + m1(2, 1) * m2(1, 2) + m1(2, 2) * m2(2, 2), m1(2, 0) * m2(0, 3) + m1(2, 1) * m2(1, 3) + m1(2, 2) * m2(2, 3) + m1(2, 3),
                     m1(3, 0) * m2(0, 0) + m1(3, 1) * m2(1, 0) + m1(3, 2) * m2(2, 0), m1(3, 0) * m2(0, 1) + m1(3, 1) * m2(1, 1) + m1(3, 2) * m2(2, 1),
                     m1(3, 0) * m2(0, 2) + m1(3, 1) * m2(1, 2) + m1(3, 2) * m2(2, 2), m1(3, 0) * m2(0, 3) + m1(3, 1) * m2(1, 3) + m1(3, 2) * m2(2, 3) + m1(3, 3)));
...
Matrix4D Terathon::operator*(const Transform3D& m1, const Matrix4D& m2)
{
    return (Matrix4D(m1(0, 0) * m2(0, 0) + m1(0, 1) * m2(1, 0) + m1(0, 2) * m2(2, 0) + m1(0, 3) * m2(3, 0),
                     ...
                     m1(2, 0) * m2(0, 3) + m1(2, 1) * m2(1, 3) + m1(2, 2) * m2(2, 3) + m1(2, 3) * m2(3, 3), m2(3, 0), m2(3, 1), m2(3, 2), m2(3, 3)));
}
...
Point3D Terathon::operator*(const Transform3D& m, const Point3D& p)
{
    ...
    return (Point3D(m(0, 0) * p.x + m(0, 1) * p.y + m(0, 2) * p.z + m(0, 3), m(1, 0) * p.x + m(1, 1) * p.y + m(1, 2) * p.z + m(1, 3),
...
{
    return (Point2D(m(0, 0) * p.x + m(0, 1) * p.y + m(0, 3), m(1, 0) * p.x + m(1, 1) * p.y + m(1, 3)));
}

Transform3D Terathon::Scale(const Transform3D& m, const Vector3D& v)
{
    return (Transform3D(m(0, 0) * v.x, m(0, 1) * v.x, m(0, 2) * v.x, m(0, 3) * v.x, m(1, 0) * v.y, m(1, 1) * v.y, m(1, 2) * v.y, m(1, 3) * v.y, m(2, 0) * v.z, m(2, 1) * v.z,
                        m(2, 2) * v.z, m(2, 3) * v.z));
}

Matrix3D Terathon::Transform(const Transform3D& m1, const Matrix3D& m2)
{
    return (Matrix3D(m1(0, 0) * m2(0, 0) + m1(0, 1) * m2(1, 0) + m1(0, 2) * m2(2, 0), m1(0, 0) * m2(0, 1) + m1(0, 1) * m2(1, 1) + m1(0, 2) * m2(2, 1),
                     m1(0, 0) * m2(0, 2) + m1(0, 1) * m2(1, 2) + m1(0, 2) * m2(2, 2), m1(1, 0) * m2(0, 0) + m1(1, 1) * m2(1, 0) + m1(1, 2) * m2(2, 0),
                     m1(1, 0) * m2(0, 1) + m1(1, 1) * m2(1, 1) + m1(1, 2) * m2(2, 1), m1(1, 0) * m2(0, 2) + m1(1, 1) * m2(1, 2) + m1(1, 2) * m2(2, 2),
                     m1(2, 0) * m2(0, 0) + m1(2, 1) * m2(1, 0) + m1(2, 2) * m2(2, 0), m1(2, 0) * m2(0, 1) + m1(2, 1) * m2(1, 1) + m1(2, 2) * m2(2, 1),
