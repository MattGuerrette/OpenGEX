...
namespace Terathon
{
    ...
    inline Bivector3D operator*(const Bivector3D&                                                                                                             v,
                                const Submat3D<type_struct, count, index_00, index_01, index_02, index_10, index_11, index_12, index_20, index_21, index_22>& m)
    {
        return (v.xyz * m);
    }
    ...
    Vector3D operator*(const Transform2D& m, const Subvec3D<type_struct, false, count, index_x, index_y, index_z>& v)
    {
        return (Vector3D(m(0, 0) * v.data[index_x] + m(0, 1) * v.data[index_y] + m(0, 2) * v.data[index_z],
                         m(1, 0) * v.data[index_x] + m(1, 1) * v.data[index_y] + m(1, 2) * v.data[index_z], v.data[index_z]));
    }

    template <typename type_struct, int count, int index_x, int index_y, int index_z>
    Antivector3D operator*(const Subvec3D<type_struct, true, count, index_x, index_y, index_z>& v, const Transform2D& m)
    {
        return (Antivector3D(m(0, 0) * v.data[index_x] + m(1, 0) * v.data[index_y], m(0, 1) * v.data[index_x] + m(1, 1) * v.data[index_y],
