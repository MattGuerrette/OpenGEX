...
namespace Terathon
{
    ...
    public:
        ...
        /// \brief Sets all four components of a 3D plane.
        /// \param n		The plane normal corresponding to the <b>e</b><sub>423</sub>, <b>e</b><sub>431</sub>, and <b>e</b><sub>412</sub> basis elements.
        /// \param p		A point on the plane. The <i>w</i> coordinate corresponding to the <b>e</b><sub>321</sub> basis element is given by
        /// &minus;(<b>n</b>&#x202F;&and;&#x202F;<b>p</b>).
    ...
    inline Plane3D operator!(const Point3D& p)
    {
        return (RightComplement(p));
    }
    inline Line3D operator!(const Line3D& l)
    {
        return (RightComplement(l));
    }
    inline FlatPoint3D operator!(const Plane3D& g)
    {
        return (RightComplement(g));
    }
    ...
    {
        return (Plane3D(l.v.y * p.z - l.v.z * p.y + l.m.x * p.w, l.v.z * p.x - l.v.x * p.z + l.m.y * p.w, l.v.x * p.y - l.v.y * p.x + l.m.z * p.w,
    ...
    {
        return (Plane3D(l.v.y * p.z - l.v.z * p.y + l.m.x, l.v.z * p.x - l.v.x * p.z + l.m.y, l.v.x * p.y - l.v.y * p.x + l.m.z, -l.m.x * p.x - l.m.y * p.y - l.m.z * p.z));
    }
    ...
    {
        return (Plane3D(l.v.y * v.z - l.v.z * v.y, l.v.z * v.x - l.v.x * v.z, l.v.x * v.y - l.v.y * v.x, -l.m.x * v.x - l.m.y * v.y - l.m.z * v.z));
    }

    inline Plane3D Wedge(const FlatPoint3D& p, const Line3D& l)
    {
        return (Wedge(l, p));
    }
    ...
    inline Line3D operator^(const FlatPoint3D& p, const FlatPoint3D& q)
    {
        return (Wedge(p, q));
    }
    ...
    inline Plane3D operator^(const Line3D& l, const FlatPoint3D& p)
    {
        return (Wedge(l, p));
    }
    ...
    inline Plane3D operator^(const FlatPoint3D& p, const Line3D& l)
    {
        return (Wedge(l, p));
    }
    ...
    {
        return (FlatPoint3D(l.m.y * g.z - l.m.z * g.y + l.v.x * g.w, l.m.z * g.x - l.m.x * g.z + l.v.y * g.w, l.m.x * g.y - l.m.y * g.x + l.v.z * g.w,
    ...
    inline FlatPoint3D Antiwedge(const Line3D& l, const Plane3D& g)
    {
        return (Antiwedge(g, l));
    }

    inline Line3D operator^(const Plane3D& g, const Plane3D& h)
    {
        return (Antiwedge(g, h));
    }
    inline FlatPoint3D operator^(const Plane3D& g, const Line3D& l)
    {
        return (Antiwedge(g, l));
    }
    inline float operator^(const Line3D& k, const Line3D& l)
    {
        return (Antiwedge(k, l));
    }
    inline float operator^(const FlatPoint3D& p, const Plane3D& g)
    {
        return (Antiwedge(p, g));
    }
    ...
    inline FlatPoint3D operator^(const Line3D& l, const Plane3D& g)
    {
        return (Antiwedge(g, l));
    }
