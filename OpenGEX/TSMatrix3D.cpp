...
Matrix3D Matrix3D::MakeRotation(float angle, const Bivector3D& axis)
{
    ...
    return (
        Matrix3D(c + x * axis.x, axay - s * axis.z, axaz + s * axis.y, axay + s * axis.z, c + y * axis.y, ayaz - s * axis.x, axaz - s * axis.y, ayaz + s * axis.x, c + z * axis.z));
}
...
Matrix3D Matrix3D::MakeReflection(const Vector3D& a)
{
    ...
    return (Matrix3D(x * a.x + 1.0F, axay, axaz, axay, y * a.y + 1.0F, ayaz, axaz, ayaz, z * a.z + 1.0F));
}
...
Matrix3D Matrix3D::MakeSkew(float angle, const Vector3D& a, const Vector3D& b)
{
    ...
    return (Matrix3D(x * b.x + 1.0F, x * b.y, x * b.z, y * b.x, y * b.y + 1.0F, y * b.z, z * b.x, z * b.y, z * b.z + 1.0F));
}
...
Matrix3D Terathon::Inverse(const Matrix3D& m)
{
    ...
    return (Matrix3D(g0.x * invDet, g0.y * invDet, g0.z * invDet, g1.x * invDet, g1.y * invDet, g1.z * invDet, g2.x * invDet, g2.y * invDet, g2.z * invDet));
}
...
Transform2D Transform2D::MakeRotation(float angle)
{
    ...
    return (Transform2D(v.x, -v.y, 0.0F, v.y, v.x, 0.0F));
}
...
{
    return (Transform2D(m1(0, 0) * m2(0, 0) + m1(0, 1) * m2(1, 0), m1(0, 0) * m2(0, 1) + m1(0, 1) * m2(1, 1), m1(0, 0) * m2(0, 2) + m1(0, 1) * m2(1, 2) + m1(0, 2),
...
{
    return (Matrix3D(m1(0, 0) * m2(0, 0) + m1(0, 1) * m2(1, 0), m1(0, 0) * m2(0, 1) + m1(0, 1) * m2(1, 1), m1(0, 0) * m2(0, 2) + m1(0, 1) * m2(1, 2) + m1(0, 2),
                     m1(1, 0) * m2(0, 0) + m1(1, 1) * m2(1, 0), m1(1, 0) * m2(0, 1) + m1(1, 1) * m2(1, 1), m1(1, 0) * m2(0, 2) + m1(1, 1) * m2(1, 2) + m1(1, 2),
                     m1(2, 0) * m2(0, 0) + m1(2, 1) * m2(1, 0), m1(2, 0) * m2(0, 1) + m1(2, 1) * m2(1, 1), m1(2, 0) * m2(0, 2) + m1(2, 1) * m2(1, 2) + m1(2, 2)));
}
...
Matrix3D Terathon::operator*(const Transform2D& m1, const Matrix3D& m2)
{
    return (Matrix3D(m1(0, 0) * m2(0, 0) + m1(0, 1) * m2(1, 0) + m1(0, 2) * m2(2, 0), m1(0, 0) * m2(0, 1) + m1(0, 1) * m2(1, 1) + m1(0, 2) * m2(2, 1),
                     ...
                     m1(1, 0) * m2(0, 1) + m1(1, 1) * m2(1, 1) + m1(1, 2) * m2(2, 1), m1(1, 0) * m2(0, 2) + m1(1, 1) * m2(1, 2) + m1(1, 2) * m2(2, 2), m2(2, 0), m2(2, 1),
                     m2(2, 2)));
}
...
{
    return (Point2D(m(0, 0) * p.x + m(0, 1) * p.y + m(0, 2), m(1, 0) * p.x + m(1, 1) * p.y + m(1, 2)));
}
...
Transform2D Terathon::Inverse(const Transform2D& m)
{
    ...
    return (Transform2D(m(1, 1) * invDet, -m(0, 1) * invDet, (m(0, 1) * m(1, 2) - m(0, 2) * m(1, 1)) * invDet, -m(1, 0) * invDet, m(0, 0) * invDet,
                        (m(0, 2) * m(1, 0) - m(0, 0) * m(1, 2)) * invDet));
}

Transform2D Terathon::InverseUnitDet(const Transform2D& m)
{
    return (Transform2D(m(1, 1), -m(0, 1), m(0, 1) * m(1, 2) - m(0, 2) * m(1, 1), -m(1, 0), m(0, 0), m(0, 2) * m(1, 0) - m(0, 0) * m(1, 2)));
}
